...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    int num = len - numToKeep;
    while (num--) {
        string tmp = pq.dequeue();
        mp.remove(tmp);
    }
    return mp;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    string name = "";
    double mx = 0;
    for (Corpus s : corpora) {
        if (mx < cosineSimilarityOf(textProfile, s.profile)) {
            name = s.name;
            mx = cosineSimilarityOf(textProfile, s.profile);
        }
    }
    return name;
}
